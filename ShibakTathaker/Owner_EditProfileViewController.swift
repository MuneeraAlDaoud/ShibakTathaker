...
class Owner_EditProfileViewController: UIViewController {
    ...
    @IBAction func update(_ sender: Any) {
        ...
        let queryString1 = "Update owners set username = ?, organization_name = ?, email = ?, phone = ? where username = '"+username+"'"
        ...
        if sqlite3_bind_text(stmt1, 2, Organization_name, -1, SQLITE_TRANSIENT) != SQLITE_OK{
            let errmsg = String(cString: sqlite3_errmsg(db)!)
            print("failure binding organization name: \(errmsg)")
            return
        }
        
        if sqlite3_bind_text(stmt1, 3, Owner_email, -1, SQLITE_TRANSIENT) != SQLITE_OK{
            let errmsg = String(cString: sqlite3_errmsg(db)!)
            print("failure binding owner email: \(errmsg)")
            return
        }
        
        if sqlite3_bind_int(stmt1, 4, (Owner_phone! as NSString).intValue) != SQLITE_OK{
            let errmsg = String(cString: sqlite3_errmsg(db)!)
